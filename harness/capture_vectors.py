...
def build_cookie_vectors(raw: str) -> dict:
    ...
    vectors = [
        ...
        v("wrong-secret", encode(token, sign(token, WRONG_SECRET)), "signature_invalid",
          "HMAC computed with a different secret", secret=None),
        v("truncated-sig", encode(token, sig[:-1]), "signature_invalid",
          "43 chars — one short"),
        v("overlong-sig", encode(token, sig + "A"), "signature_invalid",
          "45 chars — one long"),
        v("base64url-nopad-sig",
        ...
        v("empty-token", encode("", sign("", SECRET)), "signature_invalid",
          "valid HMAC over the empty string — verifiers must reject empty tokens before comparing"),
        v("swapped-sig", encode(token, sign(other_token, SECRET)), "signature_invalid",
          "valid signature of a different token"),
        v("dotted-token-valid", encode(dotted_token, sign(dotted_token, SECRET)),
          "signature_valid",
          "token containing a dot, correctly signed — catches first-dot splits (must split at LAST dot)"),
        v("double-urlencoded", urllib.parse.quote(raw), "signature_invalid",
